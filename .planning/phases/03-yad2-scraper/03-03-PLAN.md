...
must_haves:
  truths:
    ...
    - "Scraper returns structured Listing[] array with size_sqm field"
...
Purpose: Implement the core scraping logic to extract rental listings from Yad2 with anti-detection evasion, resilient error handling, and size_sqm field mapping.
Output: Working scraper function that returns structured Listing[] data with size_sqm from Yad2 rental search results.
</objective>
...
<task type="auto">
    ...
    **Scraping logic:**
    ...
    - Extract listings using page.evaluate():
      ...
      - For each item, extract:
        ...
        - size_sqm: .size text (parse numbers only, optional) - IMPORTANT: map to size_sqm field, not size
  ...
  <done>Scraper launches browser with stealth, navigates to Yad2, extracts listings with size_sqm field, returns structured data</done>
</task>
...
- [ ] Listings map size to size_sqm field (not size)
...
- Scraped size field is mapped to size_sqm in Listing type
- Function returns structured Listing[] array matching Phase 2 schema
