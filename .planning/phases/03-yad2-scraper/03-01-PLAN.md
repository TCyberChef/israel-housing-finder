...
must_haves:
  truths:
    ...
    - "Logger outputs structured JSON logs with timestamps"
  artifacts:
    ...
    - path: "src/scrapers/types.ts"
      provides: "Listing type definitions"
      exports: ["Listing", "ScraperResult"]
      min_lines: 20
    - path: "src/scrapers/utils/hash.ts"
      provides: "SHA-256 hash generation for deduplication"
...
Purpose: Establish the foundation for scraping Yad2 with type safety, deduplication hashing, and logging before implementing scraper logic.
Output: TypeScript config, type definitions, hash utility, logger, and installed dependencies ready for scraper implementation.
</objective>
...
<task type="auto">
  ...
  <action>
    ...
    1. src/scrapers/types.ts - Define TypeScript interfaces:
       - Listing: { id: string, address: string, city: string, price: number, rooms: number, size?: number, floor?: number, photos: string[], source_url: string, source_platform: string, source_id: string }
       - ScraperResult: { listings: Listing[], scrapedAt: string, count: number }

    2. src/scrapers/utils/hash.ts - SHA-256 hash generation:
       - Export function generateDedupeHash(address: string, rooms: number, size?: number): string
       - Use Node.js crypto.createHash('sha256')
       - Hash content format: "${address}|${rooms}|${size || 0}" (matches Phase 2 schema design)
       - Return hex digest
  ...
  <done>Type definitions exist, hash utility generates SHA-256 hashes, logger outputs structured JSON</done>
</task>
...
- [ ] src/scrapers/types.ts exports Listing and ScraperResult interfaces
...
- Type definitions for Listing and ScraperResult exist
- Hash utility produces SHA-256 hashes for deduplication
